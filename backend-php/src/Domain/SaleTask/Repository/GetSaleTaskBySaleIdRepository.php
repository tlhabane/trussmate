...
final class GetSaleTaskBySaleIdRepository
{
    ...
    public function getTaskBySaleId(string $sale_id, int $task_no = 0): PDOStatement
    {
        $query = "SELECT 
        ...
        $query .= $task_no > 0 ? " AND wt.task_no = :task_no" : "";
        $query .= " GROUP BY st.sale_task_id ORDER BY wt.task_no";

        $query_stmt = $this->connection->prepare($query);
        $query_stmt->bindParam(':sale_id', $sale_id);
        if ($task_no > 0) {
            $query_stmt->bindParam(':task_no', $task_no, PDO::PARAM_INT);
        }
        $query_stmt->execute();
