...
final class GetInvoiceByTaskIdRepository
{
    ...
    public function getInvoice(string $task_id): PDOStatement
    {
        $query = "SELECT
                  ...
                  WHERE 
                      TRIM((LOWER(i.sale_task_id))) = ? 
                  GROUP BY 
                      i.invoice_no";

        $query_stmt = $this->connection->prepare($query);
        $formatted_task_id = trim(strtolower($task_id));
